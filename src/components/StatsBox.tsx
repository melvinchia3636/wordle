...
function WinBox({
  ...
  const copyBoard = () => {
    ...
    copy(copiedBoard.map((line) => line.join('')).join('\n'));
    if (!showCopied) {
      setShowCopied(true);
      setTimeout(() => {
        setShowCopied(false);
  ...
  return (
    <>
      ...
      <div className={`flex items-center justify-center w-full h-screen absolute top-0 left-0 transition-all duration-500 ${showStats ? 'translate-y-0' : 'translate-y-[110%]'}`}>
        <div className="bg-neutral-100 dark:bg-neutral-800 text-zinc-700 dark:text-zinc-100 flex relative flex-col rounded-lg items-center justify-center w-full mx-4 510:mx-16 lg:w-1/2 py-8 shadow-2xl">
          ...
          <div className="flex flex-col gap-2 w-full px-6 510:px-20">
            {stats.guess.map((e, i) => (
              <div className="flex items-center gap-2">
                <span className="font-semibold w-4">{i + 1}</span>
                <div
                  className={`w-8 rounded-md p-1 px-3 ${isFinished && currentLine - 1 === i && stats.streak > 0 ? (highContrast ? 'bg-sky-400' : 'bg-lime-500') : 'bg-neutral-200 dark:bg-neutral-600'} font-semibold`}
                  style={{
          ...
          {isFinished && (
            <div className="flex items-center gap-2 w-full px-6 510:px-12 mt-8">
      ...
      <div className={`p-4 px-8 bg-neutral-200 dark:bg-neutral-700 font-medium rounded-md shadow-md text-neutral-600 dark:text-neutral-100 absolute z-10 top-6 left-1/2 -translate-x-1/2 flex whitespace-nowrap items-center transition-transform duration-500 gap-2 ${showCopied ? 'translate-y-0' : '-translate-y-[150%]'}`}>
        <Icon icon="ph:copy" className={`w-6 h-6 flex-shrink-0 ${highContrast ? 'text-sky-400' : 'text-lime-500'}`} />
