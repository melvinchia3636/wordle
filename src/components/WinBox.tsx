...
function WinBox({
  board, getBoardColor, isWin, newGame, highContrast,
}: {
  board: string[][];
  ...
  const [winboxShow, setWinboxShow] = useState(false);
  ...
  return (
    <>
      <div className={`w-full h-screen bg-neutral-900 transition-colors duration-500 absolute top-0 left-0 ${isWin && winboxShow ? 'bg-opacity-50 z-0' : 'bg-opacity-0 z-[-1]'}`} />
      <div className={`flex items-center justify-center w-full h-screen absolute top-0 left-0 transition-all duration-500 ${isWin && winboxShow ? 'translate-y-0' : 'translate-y-[110%]'}`}>
        <div className="bg-neutral-100 dark:bg-neutral-800 text-zinc-700 dark:text-zinc-100 flex relative flex-col rounded-lg items-center justify-center w-1/2 py-8 shadow-2xl">
          ...
          <h2 className="font-medium text-4xl mb-6">Statistic</h2>
